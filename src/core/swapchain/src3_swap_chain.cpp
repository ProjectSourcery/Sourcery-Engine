...
namespace src3 {
    ...
    SrcSwapChain::SrcSwapChain(SrcDevice& deviceRef, VkExtent2D extent)
        : device{ deviceRef }, windowExtent{ extent } {
        init();
    }

    SrcSwapChain::SrcSwapChain(SrcDevice& deviceRef, VkExtent2D extent, std::shared_ptr<SrcSwapChain> previous)
    ...
    SrcSwapChain::~SrcSwapChain() {
        ...
        for (long long unsigned int i = 0; i < depthImages.size(); i++) {
    ...
    VkResult SrcSwapChain::acquireNextImage(uint32_t* imageIndex) {
        vkWaitForFences(
            device.device(),
            1,
    ...
    void SrcSwapChain::createSwapChain() {
    ...
    void SrcSwapChain::createImageViews() {
    ...
    void SrcSwapChain::createRenderPass() {
    ...
    void SrcSwapChain::createViewportRenderPass() {
        VkAttachmentDescription depthAttachment{};
        depthAttachment.format = findDepthFormat();
        depthAttachment.samples = VK_SAMPLE_COUNT_1_BIT;
        depthAttachment.loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR;
        ...
        dependency2.srcAccessMask =  VK_ACCESS_MEMORY_READ_BIT;
        ...
        if (vkCreateRenderPass(device.device(), &renderPassInfo, nullptr, &viewportRenderPass) != VK_SUCCESS) {
            throw std::runtime_error("failed to create render pass!");
        }
    }

    void SrcSwapChain::createFramebuffers() {
    ...
    void SrcSwapChain::createViewportFramebuffers() {
        swapChainViewportFramebuffers.resize(viewportImageViews.size());
    ...
    void SrcSwapChain::createViewportResources() {
        ...
        for (long long unsigned int i = 0; i < viewportImages.size(); i++) {
            ...
            VkImageViewCreateInfo viewInfo{};
            viewInfo.sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO;
            viewInfo.image = viewportImages[i];
            viewInfo.viewType = VK_IMAGE_VIEW_TYPE_2D;
            ...
            if (vkCreateImageView(device.device(), &viewInfo, nullptr, &viewportImageViews[i]) != VK_SUCCESS) {
                throw std::runtime_error("failed to create texture image view!");
            }
        }
    }

    void SrcSwapChain::createSyncObjects() {
    ...
    VkSurfaceFormatKHR SrcSwapChain::chooseSwapSurfaceFormat(
    ...
    VkExtent2D SrcSwapChain::chooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities) {
    ...
    VkFormat SrcSwapChain::findDepthFormat() {
