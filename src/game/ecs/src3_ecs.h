...
class EntQuery {
 ...
 private:
  ...
  EntQuery__internal build() {
    // calculate hashcode
    size_t hashValue = 0;
    for (const auto componentType : allOfTypes) src3::hashCombine(hashValue, componentType);
    for (const auto componentType : anyOfTypes) src3::hashCombine(hashValue, componentType);
    for (const auto componentType : noneOfTypes) src3::hashCombine(hashValue, componentType);
