...
static bool assertFailedImpl(const char *inExpr, const char *inMessage, const char *inFile, uint inLine) {
    cout << "[ASSERT_F][" << inFile << "]: (" << inExpr << ") " << (inMessage != nullptr? inMessage : "") << endl;
...
namespace src3 {
    ...
    class SrcPhysicsSystem {
        public:
            SrcPhysicsSystem(entt::registry &entityComponentSystem);
            ~SrcPhysicsSystem();
        ...
        private:
            ...
            ObjectVsBroadPhaseLayerFilterImpl objectVsBroadpahseLayerFilter;
