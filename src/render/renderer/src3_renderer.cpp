...
namespace src3 {
    ...
    void SrcRenderer::beginRenderPass(VkCommandBuffer commandBuffer, VkRenderPass rendPass, VkFramebuffer framebuffer, std::vector<VkClearValue> clearValues)
    {
        assert(isFrameStarted && "Can't call beginRenderPass while frame is not in progress");

        VkRenderPassBeginInfo renderPassInfo{};
        renderPassInfo.sType = VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO;
        renderPassInfo.renderPass = rendPass;
    ...
    void SrcRenderer::endRenderPass(VkCommandBuffer commandBuffer)
    {
        assert(isFrameStarted && "Can't call endRenderPass while frame is not in progress");
        vkCmdEndRenderPass(commandBuffer);
    }
}