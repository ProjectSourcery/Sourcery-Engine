...
namespace src3 {
SrcTexture::SrcTexture(SrcDevice &device, const std::string &textureFilepath) : mDevice{device} {
...
std::unique_ptr<SrcTexture> SrcTexture::createTextureFromFile(
    SrcDevice &device, const std::string &filepath) {
  return std::make_unique<SrcTexture>(device, filepath);
}

void SrcTexture::updateDescriptor() {
  mDescriptor.sampler = mTextureSampler;
  mDescriptor.imageView = mTextureImageView;
  mDescriptor.imageLayout = mTextureLayout;
}

void SrcTexture::createTextureImage(const std::string &filepath) {
...
void SrcTexture::createTextureImageView(VkImageViewType viewType) {
  VkImageViewCreateInfo viewInfo{};
  viewInfo.sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO;
  viewInfo.image = mTextureImage;
...
void SrcTexture::createTextureSampler() {
  VkSamplerCreateInfo samplerInfo{};
  samplerInfo.sType = VK_STRUCTURE_TYPE_SAMPLER_CREATE_INFO;
  samplerInfo.magFilter = VK_FILTER_LINEAR;
  samplerInfo.minFilter = VK_FILTER_LINEAR;
...
void SrcTexture::transitionLayout(
